...
This repository now includes a dedicated OpenEnv submission surface for the hackathon.

### Environment Summary

- Domain: Urban infrastructure / municipal waste logistics
- Runtime: Deterministic in-memory simulator with one-step episodes
- Server entrypoint: [app.py](app.py)
- Environment implementation: [env/infrawatch_env.py](env/infrawatch_env.py)
- Manifest: [openenv.yaml](openenv.yaml)

### Tasks

- `task1_triage`: rank critical and escalated bins in order
- `task2_route`: optimize a single-ward collection route
- `task3_allocation`: allocate trucks across wards under capacity limits

### Action Space

- `triage`: submit ordered `priority_bins`
- `route`: submit `ward` plus ordered `route`
- `allocate`: submit `assignments` per truck

### Observation Space

The observation returned by `/reset` and embedded in `/step` contains:
...
`inference.py` uses the OpenAI client and the required environment variables:

- `API_BASE_URL`
- `MODEL_NAME`
- `HF_TOKEN`
- `ENV_URL` or `CIVIC_ENV_URL` for local testing
...
Run the pre-submission checker before publishing:
...
- Hugging Face Space responds `200` on `GET /`
- `GET /health` responds `200`
- `POST /reset`, `POST /step`, and `GET /state` work for all three tasks
- `openenv.yaml` includes all tasks and grader paths
- Rewards stay in `0.0` to `1.0`
- `inference.py` uses OpenAI client with `API_BASE_URL`, `MODEL_NAME`, and `HF_TOKEN`
- Inference emits JSON single-line `[START]`, `[STEP]`, `[END]` logs
- Docker image builds and runs `uvicorn app:app --port 7860`
- `python -m pytest -q tests/test_env_compliance.py tests/test_graders.py tests/test_scenarios.py tests/test_inference.py` passes

### Git Push Note (Important)

The remote default branch is `main`, while this local branch was `master`.
Push to `main` before final submission so evaluators and README updates are visible on the default branch.
